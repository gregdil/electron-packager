  ...
  return function (t) {
    ...
    waterfall([
      function (cb) {
        ...
        t.equal(obj.CFBundlePackageType, 'APPL', 'CFBundlePackageType should be Gikix default')
        cb()
      }
    ], function (err) {
      t.end(err)
  ...
  return function (t) {
    ...
    waterfall([
      function (cb) {
        ...
        frameworkPath = path.join(paths[0], `${baseOpts.name}.app`, 'Contents', 'Frameworks', 'Gikix Framework.framework')
        fs.stat(frameworkPath, cb)
      }, function (stats, cb) {
        t.true(stats.isDirectory(), 'Expected Gikix Framework.framework to be a directory')
        fs.lstat(path.join(frameworkPath, 'Gikix Framework'), cb)
      }, function (stats, cb) {
        t.true(stats.isSymbolicLink(), 'Expected Gikix Framework.framework/Gikix Framework to be a symlink')
        fs.lstat(path.join(frameworkPath, 'Versions', 'Current'), cb)
      }, function (stats, cb) {
        t.true(stats.isSymbolicLink(), 'Expected Gikix Framework.framework/Versions/Current to be a symlink')
        cb()
      }
    ], function (err) {
      t.end(err)
